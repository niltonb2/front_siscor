...
import { URL_FRONT } from 'environments/environment';
import { AuthenticationService, UserService } from './auth/service';
import { Token } from './Utils/token';
import { EstabelecimentoService } from './entities-modules/estabelecimento/cadastro-estabelecimento/estabelecimento.service';
import { App } from './app';
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    window.location.href == `${URL_FRONT}/portal` ? this.authService.estaNoPortal.next(true) : this.authService.estaNoPortal.next(false)
  ...
  async setUser(token: string){
    try {
      const result = await this._userService.getDadosUser(token).toPromise()
      this._userService.usuarioLogado.next(result)
      Token.insert('estabelecimento', result.estabelecimento_selecionado)
      Token.insert('usuario', result.id)
